...
const CategoryPage: React.FC = () => {
  ...
  if (!category && categories.length > 0) {
      return (
          <div className="min-h-[50vh] flex flex-col items-center justify-center p-10">
              <h2 className="text-2xl font-black text-gray-900 dark:text-white mb-4">الفئة غير موجودة</h2>
              <button 
                  onClick={() => navigate('/')}
  ...
  const categoryProducts = products.filter(p => 
    ...
    p.isActive &&
    (searchTerm === '' || p.name.en.toLowerCase().includes(searchTerm.toLowerCase()) || p.name.ar.includes(searchTerm))
  );
    ...
    <div className="animate-in fade-in duration-700">
      ...
      <section className="bg-orange-50 dark:bg-gray-900/30 pt-20 pb-32 border-b-4 border-white dark:border-gray-800 relative overflow-hidden">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 md:py-12 relative z-10">
            ...
            <div className="flex flex-col md:flex-row items-center gap-8">
                <div className="w-32 h-32 md:w-40 md:h-40 rounded-[2.5rem] overflow-hidden shadow-2xl border-4 border-white dark:border-gray-800">
                    <img src={category?.image} className="w-full h-full object-cover" alt={category?.name[language]} />
                </div>
                <div className="text-center md:text-right">
                    ...
                    <p className="text-xl text-gray-500 dark:text-gray-400 font-medium">
                        استكشف أحدث المنتجات في قسم {category?.name[language]}
                    </p>
                </div>
            </div>
        </div>
      ...
      {selectedProduct && (
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
             ...
             <div className="relative bg-white dark:bg-gray-900 w-full max-w-lg p-10 rounded-3xl z-10 text-center">
